...
function Toggle({ value, onChange }: { value: boolean; onChange: () => void }) {
  return (
    <button onClick={onChange}
      className={`w-10 rounded-full transition-colors relative flex-shrink-0 ${value ? "bg-[#3B82F6]" : "bg-slate-200"}`}
      style={{ height: "22px", width: "40px" }}>
      <span className="absolute top-[2px] w-[18px] h-[18px] bg-white rounded-full shadow-sm transition-all"
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <motion.div initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.96 }}
        ...
        <div className="p-6 space-y-4">
          {/* Name */}
          <div>
            <label className="block text-slate-700 text-sm mb-1.5" style={{ fontWeight: 600 }}>Department Name <span className="text-red-500">*</span></label>
          ...
          <div>
            ...
            <textarea value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))}
              ...
              rows={3}
              className="w-full bg-[#F8FAFC] border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-[#3B82F6] focus:ring-2 focus:ring-[#3B82F6]/10 transition-all resize-none" />
          </div>

          {/* Department Head */}
          <div>
            <label className="block text-slate-700 text-sm mb-1.5" style={{ fontWeight: 600 }}>Department Head</label>
            <div className="relative">
              <select value={form.head} onChange={e => setForm(f => ({ ...f, head: e.target.value }))}
                className="w-full appearance-none bg-[#F8FAFC] border border-slate-200 rounded-xl px-4 py-2.5 pr-8 text-sm focus:outline-none focus:border-[#3B82F6] focus:ring-2 focus:ring-[#3B82F6]/10 transition-all">
          ...
          <div className="flex items-center justify-between p-4 bg-[#F8FAFC] border border-slate-200 rounded-xl">
            <div>
              <p className="text-slate-800 text-sm" style={{ fontWeight: 500 }}>Department Status</p>
              <p className="text-slate-400 text-xs">Active departments are visible to staff</p>
            </div>
  ...
  return (
    <div className="p-6 lg:p-8 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-slate-900" style={{ fontSize: "1.4rem", fontWeight: 800 }}>Department Configuration</h1>
          <p className="text-slate-500 text-sm mt-0.5">{depts.filter(d => d.status).length} active departments</p>
        </div>
      ...
      <div className="bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden">
        <div className="hidden md:grid grid-cols-[2fr,2.5fr,1.5fr,auto,auto,auto] gap-4 px-6 py-3 bg-slate-50 border-b border-slate-100">
          ...
          {filtered.map((d, i) => {
            ...
            return (
              <motion.div key={d.id}
                ...
                className="flex flex-col md:grid md:grid-cols-[2fr,2.5fr,1.5fr,auto,auto,auto] gap-2 md:gap-4 items-start md:items-center px-6 py-4 hover:bg-slate-50/60 transition-colors">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-slate-900 text-sm" style={{ fontWeight: 600 }}>{d.name}</span>
                </div>

                {/* Description */}
                <p className="text-slate-500 text-xs leading-relaxed hidden md:block">{d.description}</p>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-slate-700 text-xs">{d.head}</span>
                </div>

                {/* Staff count */}
                <div className="flex items-center gap-1.5 hidden md:flex">
                  <Users className="w-3.5 h-3.5 text-slate-400" />
                  <span className="text-slate-700 text-sm" style={{ fontWeight: 600 }}>{d.staffCount}</span>
                </div>

                {/* Status toggle */}
                <div className="flex items-center gap-2">
                  <Toggle value={d.status} onChange={() => toggleStatus(d.id)} />
                  <span className={`text-xs ${d.status ? "text-[#10B981]" : "text-slate-400"}`} style={{ fontWeight: 500 }}>
                ...
                <div className="flex items-center gap-1 relative">
                  <button onClick={() => setFormDept(d)} className="p-1.5 text-slate-400 hover:text-[#3B82F6] hover:bg-[#EFF6FF] rounded-lg transition-colors">
