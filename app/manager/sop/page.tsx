...
const statusConfig: Record<string, { bg: string; icon: typeof CheckCircle2; color: string }> = {
  active: { bg: "bg-[#F0FDF4] text-[#10B981]", icon: CheckCircle2, color: "#10B981" },
  review: { bg: "bg-[#FFFBEB] text-[#F59E0B]", icon: Clock, color: "#F59E0B" },
  draft: { bg: "bg-slate-100 text-slate-500", icon: AlertCircle, color: "#94A3B8" },
  archived: { bg: "bg-[#FEF2F2] text-[#EF4444]", icon: Archive, color: "#EF4444" },
};
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      ...
      >
        ...
        <div className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
          {/* Title */}
          <div>
            <label className="block text-slate-700 text-sm mb-1.5" style={{ fontWeight: 600 }}>SOP Title <span className="text-red-500">*</span></label>
            <input
              value={form.title} onChange={e => { setForm(f => ({ ...f, title: e.target.value })); setError(""); }}
              placeholder="e.g. Front Desk Check-In Procedure"
              className={`w-full bg-[#F8FAFC] border rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-[#3B82F6] focus:ring-2 focus:ring-[#3B82F6]/10 transition-all ${error ? "border-red-300" : "border-slate-200"}`}
            />
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-slate-700 text-sm mb-1.5" style={{ fontWeight: 600 }}>Department</label>
              <div className="relative">
                <select
                  value={form.dept} onChange={e => setForm(f => ({ ...f, dept: e.target.value }))}
                  className="w-full appearance-none bg-[#F8FAFC] border border-slate-200 rounded-xl px-4 py-2.5 pr-8 text-sm focus:outline-none focus:border-[#3B82F6] transition-all"
                >
            ...
            <div>
              <label className="block text-slate-700 text-sm mb-1.5" style={{ fontWeight: 600 }}>Status</label>
              <div className="relative">
                <select
          ...
          {/* Version */}
          <div>
            <label className="block text-slate-700 text-sm mb-1.5" style={{ fontWeight: 600 }}>Version</label>
            <input
              value={form.version} onChange={e => setForm(f => ({ ...f, version: e.target.value }))}
              placeholder="v1.0"
              className="w-full bg-[#F8FAFC] border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-[#3B82F6] transition-all"
            />
          </div>

          {/* Description */}
          <div>
            <label className="block text-slate-700 text-sm mb-1.5" style={{ fontWeight: 600 }}>Description</label>
            <textarea
              value={form.description ?? ""} onChange={e => setForm(f => ({ ...f, description: e.target.value }))}
              placeholder="Brief description of this SOP's purpose..." rows={2}
              className="w-full bg-[#F8FAFC] border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-[#3B82F6] transition-all resize-none"
            />
          </div>
          ...
          <div>
            <label className="flex items-center gap-1.5 text-slate-700 text-sm mb-2" style={{ fontWeight: 600 }}>
              <Users className="w-3.5 h-3.5 text-slate-400" /> Assign to Departments
            </label>
            <div className="flex flex-wrap gap-2">
              {ASSIGN_OPTIONS.map(d => (
                <button
                  key={d} onClick={() => toggleDept(d)}
                  className={`px-3 py-1.5 rounded-lg border text-xs transition-all ${form.assignedTo.includes(d) ? "bg-[#EFF6FF] border-[#3B82F6] text-[#3B82F6]" : "bg-[#F8FAFC] border-slate-200 text-slate-500 hover:border-slate-300"}`}
        ...
        <div className="px-6 py-4 border-t border-slate-100 bg-slate-50 flex gap-3">
          <button onClick={onClose} className="px-5 py-2.5 rounded-xl border border-slate-200 text-slate-600 text-sm hover:bg-slate-100 transition-colors" style={{ fontWeight: 600 }}>
            Cancel
          </button>
          <button
            onClick={handleSave}
            className="flex-1 py-2.5 rounded-xl bg-gradient-to-r from-[#3B82F6] to-[#4F46E5] text-white text-sm shadow-md"
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={onClose}>
      ...
      >
        <div className="flex items-center justify-between px-6 py-5 border-b border-slate-100 bg-gradient-to-r from-[#0B1628] to-[#1E293B]">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-slate-400 text-xs">{sop.dept}</p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-6">
          <h2 className="text-slate-900 mb-3" style={{ fontWeight: 800, fontSize: "1.1rem" }}>{sop.title}</h2>
          ...
          {sop.status === "active" && (
            <div className="bg-white border border-slate-100 rounded-xl p-4 mb-5 shadow-sm">
              <div className="flex items-center justify-between mb-2">
                <span className="text-xs text-slate-500 flex items-center gap-1.5"><CheckCircle2 className="w-3.5 h-3.5 text-[#10B981]" /> Compliance</span>
            ...
            <div>
              ...
              <div className="space-y-2.5">
                {sop.steps.map((step, idx) => (
                  <div key={step.id} className="flex items-start gap-3 bg-slate-50 rounded-xl px-4 py-3">
    ...
    <div className="p-6 lg:p-8 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-0.5">{sops.filter(s => s.status === "active").length} active SOPs across all departments</p>
        </div>
        <motion.button
      ...
      <div className="bg-white rounded-xl p-4 border border-slate-100 shadow-sm flex flex-col md:flex-row gap-3">
        ...
        <div className="flex gap-2">
          <div className="relative">
            <select value={deptFilter} onChange={e => setDeptFilter(e.target.value)} className="appearance-none bg-slate-50 border border-slate-200 rounded-lg px-4 py-2.5 pr-8 text-sm focus:outline-none focus:border-[#3B82F6] transition-all">
              {deptsList.map(d => <option key={d}>{d}</option>)}
            </select>
            <ChevronDown className="absolute right-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-slate-400 pointer-events-none" />
          </div>
          <div className="relative">
            <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)} className="appearance-none bg-slate-50 border border-slate-200 rounded-lg px-4 py-2.5 pr-8 text-sm focus:outline-none focus:border-[#3B82F6] transition-all capitalize">
      ...
      <div className="bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden">
        <div className="hidden md:grid grid-cols-[2.5fr,1fr,1fr,1fr,1fr,auto] gap-4 px-6 py-3 bg-slate-50 border-b border-slate-100">
          ...
          {filtered.map((sop, i) => {
            ...
            return (
              <motion.div
                ...
                className="flex flex-col md:grid md:grid-cols-[2.5fr,1fr,1fr,1fr,1fr,auto] gap-2 md:gap-4 items-start md:items-center px-6 py-4 hover:bg-slate-50/60 transition-colors"
               >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-slate-900 text-sm" style={{ fontWeight: 600 }}>{sop.title}</p>
                    {sop.assignedTo.length > 0 && (
                      <p className="text-slate-400 text-xs mt-0.5">Assigned to: {sop.assignedTo.join(", ")}</p>
                    )}
                  </div>
                </div>
                <span className="text-slate-600 text-xs bg-slate-100 px-2.5 py-1 rounded-full" style={{ fontWeight: 500 }}>{sop.dept}</span>
                <span className="text-slate-500 text-sm hidden md:block">{sop.version}</span>
                <span className="text-slate-500 text-xs hidden md:block">{sop.updated}</span>
                ...
                <div className="flex items-center gap-1">
                  ...
                  <button onClick={() => setModalSop(sop)} className="p-1.5 text-slate-400 hover:text-[#3B82F6] hover:bg-[#EFF6FF] rounded-lg transition-colors" title="Edit">
