...
const statusConfig = {
  done: { color: "#10B981", bg: "bg-black/[0.04] text-black", label: "Done" },
  pending: { color: "#F59E0B", bg: "bg-black/[0.04] text-neutral-600", label: "Pending" },
  upcoming: { color: "#3B82F6", bg: "bg-black/[0.04] text-black", label: "Upcoming" },
};
  ...
  return (
    <div className="p-6 lg:p-8 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-black" style={{ fontSize: "1.4rem", fontWeight: 800 }}>Manager Dashboard</h1>
          <p className="text-neutral-500 text-sm mt-0.5">Skyline Suites — Today's operations</p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {stats.map((s, i) => (
          ...
          >
            ...
            <div className="text-black" style={{ fontSize: "1.6rem", fontWeight: 800 }}>{s.value}</div>
            <div className="text-neutral-500 text-sm mt-0.5">{s.label}</div>
            <div className={`text-xs mt-2 ${s.positive ? "text-black" : "text-neutral-600"}`}>{s.change}</div>
          </motion.div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-5">
        {/* Tasks */}
        <div className="lg:col-span-3 bg-white/70 backdrop-blur rounded-2xl border border-black/10 shadow-sm overflow-hidden">
          <div className="flex items-center justify-between px-6 py-5 border-b border-black/10">
            <h3 className="text-black" style={{ fontWeight: 700 }}>Today's Tasks</h3>
            <button className="flex items-center gap-1.5 text-black text-sm hover:underline">
          ...
          <div className="divide-y divide-slate-50">
            {tasks.map((t, i) => {
              ...
              return (
                <div key={i} className="flex items-center gap-4 px-6 py-4 hover:bg-white/50/50 transition-colors">
                  <div className={`w-2 h-2 rounded-full flex-shrink-0`} style={{ backgroundColor: cfg.color }} />
                  <div className="flex-1 min-w-0">
                    <p className="text-black text-sm truncate">{t.task}</p>
                    <p className="text-neutral-400 text-xs mt-0.5">{t.assignee} · Due {t.due}</p>
                  </div>
        ...
        {/* Attendance */}
        <div className="lg:col-span-2 bg-white/70 backdrop-blur rounded-2xl border border-black/10 shadow-sm overflow-hidden">
          <div className="flex items-center justify-between px-6 py-5 border-b border-black/10">
            <h3 className="text-black" style={{ fontWeight: 700 }}>Staff Today</h3>
            <span className="text-xs text-neutral-500">12/15 present</span>
          </div>
          <div className="divide-y divide-slate-50">
            {staffAttendance.map((s, i) => (
              <div key={i} className="flex items-center gap-3 px-6 py-3.5">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-black text-sm" style={{ fontWeight: 600 }}>{s.name}</p>
                  <p className="text-neutral-400 text-xs">{s.dept}</p>
                </div>
                <div className="text-right">
                  <span className={`text-xs ${s.status === "in" ? "text-black" : "text-black"}`} style={{ fontWeight: 600 }}>
      ...
      <div className="bg-white/70 backdrop-blur rounded-2xl p-6 border border-black/10 shadow-sm">
        <h3 className="text-black mb-5" style={{ fontWeight: 700 }}>Task Completion — This Week</h3>
